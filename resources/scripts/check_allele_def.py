...
def  write_log(title:str,logger:str,file_name:str='name_log',):
    t_bar = '+'*80 + '\n'
    leaft = 15*'%'
...
def set_none_rsid(ph_cat_def_set):
...
def find_conflict(a_set:set,b_set:set):
    a_set_conflict = []
    in_both = []
...
def  compare_allele_name(gene:str,cat_def:set,vip_def:set):
    cat_allele = set()
    vip_allele = set()
    for allele in cat_def[gene]["namedAlleles"]:
        allele_name = allele['name']
        cat_allele.add(allele_name)
    for allele in vip_def[gene]["name_relation_to_hgvs"] :
        allele_name = allele['name']
    ...
    if vip_allele == cat_allele:
        ...
        in_both, cat_conflict, vip_conflict = find_conflict(cat_allele,vip_allele)
        logger = get_conflict_log(in_both=in_both,
            cat_conflict=cat_conflict,
            vip_conflict=vip_conflict,
            which_conflict="GENE")

    write_log(title=gene, logger=logger, file_name='compare_allele.log')
    
def  compare_position(gene:str, cat_def:set, vip_def:set,is_write:bool=True):
    cat_pos = {}
    cat_pos['chromosome'] = cat_def[gene]["variants"][0]["chromosome"]
    cat_pos['variants'] = []
    for variant in cat_def[gene]["variants"]:
        data_pack = dict()
    ...
    #%%
    vip_pos = {}
    vip_pos['chromosome'] = vip_def[gene]["haplotypes"][0]["chromosome"]
    vip_pos['variants'] = []
    logger = ''
    for variant in vip_def[gene]["haplotypes"][0]["variants"]:
        data_pack = {}
    ...
    if vip_pos == cat_pos:
        ...
        if vip_pos['variants'] != cat_pos['variants']:
            ...
            in_both, cat_conflict, vip_conflict  = find_conflict(cat_variants,vip_variants)
            logger = logger + get_conflict_log(in_both=in_both,
                cat_conflict=cat_conflict,
                vip_conflict=vip_conflict,
                which_conflict="variant")

    if is_write : write_log(title=gene, logger=logger, file_name='compare_possition.log')
    return cat_pos, vip_pos

def get_conflict_log(in_both:set, 
        cat_conflict:set, 
        vip_conflict:set,
        which_conflict:str=''):
...
def  check_gene(cat_def:dict,vip_def:dict,ilog:bool=True):
    cat_gene = {gene for gene in cat_def}
    vip_gene = {gene for gene in vip_def}
    ...
    if cat_gene == vip_gene :
        ...
        in_both, cat_conflict, vip_conflict = find_conflict(cat_gene,vip_gene)
        logger = get_conflict_log(in_both=in_both,
            cat_conflict=cat_conflict,
            vip_conflict=vip_conflict,
            which_conflict="GENE")
        
    if ilog: write_log(title="Compare Gene",logger=logger,file_name='compare_gene.log')
...
set_none_rsid(ph_cat_def_set)
in_both, cat_conflict, vip_conflict = check_gene(cat_def=ph_cat_def_set,vip_def=ph_vip_def_set)
for gene in in_both:
    # compare_allele_name(gene=gene,
    #     cat_def=ph_cat_def_set,
    #     vip_def=ph_vip_def_set)
    cat, vip = compare_position(gene=gene,
        cat_def=ph_cat_def_set,
        vip_def=ph_vip_def_set)

# %%
gene='CYP2C19'
x = ph_cat_def_set[gene]['variants']
cat, vip = compare_position(gene=gene,
        cat_def=ph_cat_def_set,
        vip_def=ph_vip_def_set,is_write=False)# %%

# # %%
# for i in  cat['variants'] :
#     pos = i['position']
#     cat_e = next((x for x in vip['variants'] if x['position'] == pos), False)
