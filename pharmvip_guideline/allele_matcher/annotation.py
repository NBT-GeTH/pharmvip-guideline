...
def read_clinical_guideline_annotations(clinical_guideline_annotations):
    ...
    _clinical_guideline_annotations = {}
    for clinical_guideline_annotation in clinical_guideline_annotations_list:
        ...
        _clinical_guideline_annotations[guideline_id] = {
    ...
    return _clinical_guideline_annotations
...
def function_mappings_diplotype_by_guideline_id(clinical_guideline_annotations, function_mappings, diplotypes):
    ...
    for guideline_id, annotations in clinical_guideline_annotations.items():
        diplotype_by_guideline_id[guideline_id] = {}
        
        queried_diplotypes = diplotypes[diplotypes["gene"].isin(annotations["related_genes"])][["gene", "guide_dip"]]
...
def annotate(_clinical_guideline_annotations, function_mappings_diplotype, diplotype):
    ...
    diplotype_new = diplotype.set_index("gene")
    ...
    for guideline_id, diplotype_by_gene in function_mappings_diplotype.items():
        if len(diplotype_by_gene) == 0:
            ...
            for gene, diplotypes in diplotype_by_gene.items():
                i = 0
                for _diplotype in diplotypes:
                    name = None
                    allele1 = _diplotype.get("allele1")
                    allele2 = _diplotype.get("allele2")
                    name = allele1.replace(gene, "") + "/" + allele2.replace(gene, "")

                    function1 = _diplotype.get("function1")
                    function2 = _diplotype.get("function2")

                    guideline_id_record = _clinical_guideline_annotations[guideline_id]["annotation_by_diplotype"].query("(Function1 == @function1) & (Function2 == @function2)")
                    
                    annotation_id = None
                    if not guideline_id_record.empty:
                        annotation_id =  guideline_id_record["GuidelineAnnotationId"].values[0]

                    guideline_annotations = _clinical_guideline_annotations[guideline_id]["guideline_annotations"]
                    ...
                    if not dosing_recommend_record.empty:
                        ...
                        summary_and_full_report['cpi_sum_dip_name1'].append(diplotype_new.loc[gene, "print_dip"][i])
                        ...
                        if gene == "HLA-A" or gene == "HLA-B":
                            summary_and_full_report['cpi_sum_hla_tool_1_guide'].append(diplotype_new.loc[gene, "tool"][i])
            ...
            for i in range(len(diplotype_list1)):
                for j in range(len(diplotype_list2)):
                    ...
                    guideline_id_record = _clinical_guideline_annotations[guideline_id]['annotation_by_diplotype'].query('(Function1 == @function1) & (Function2 == @function2) & (Function3 == @function3) & (Function4 == @function4)')
                    ...
                    if not dosing_recommend_record.empty:
                        ...
                        if gene1 == "HLA-A" or gene1 == "HLA-B":
                            summary_and_full_report['cpi_sum_hla_tool_1_guide'].append(diplotype_new.loc[gene1, "tool"][i])
                        else:
                            summary_and_full_report['cpi_sum_hla_tool_1_guide'].append('')
                        if gene2 == "HLA-A" or gene2 == "HLA-B":
                            summary_and_full_report['cpi_sum_hla_tool_2_guide'].append(diplotype_new.loc[gene2, "tool"][j])
                        else:
                            summary_and_full_report['cpi_sum_hla_tool_2_guide'].append('')
                    else:
                        summary_and_full_report['cpi_sum_gene1'].append(gene1)
...
def annotation(clinical_guideline_annotations, function_mappings, diplotype):
    _clinical_guideline_annotations = read_clinical_guideline_annotations(clinical_guideline_annotations)

    function_mappings_diplotype = function_mappings_diplotype_by_guideline_id(_clinical_guideline_annotations, function_mappings, diplotype)
    # print(json.dumps(function_mappings_diplotype, indent=2))
    
    summary_and_full_report = annotate(_clinical_guideline_annotations, function_mappings_diplotype, diplotype)
    # print(pd.DataFrame(summary_and_full_report))
    # exit()
