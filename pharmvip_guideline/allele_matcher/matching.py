...
class  AlleleMatcher() :
    def  __init__(self,allele_definitions_path:str,
        vcf_path: str,
        user_id : str = 1 ,
        id : str = 1 ) :
        """
        @param allele_definitions_path: path where allele definition Json type stored
        @vcf_path: path to in put (eg. subject of the process)
        """
        ...
        self.query_set = {}
        for allele_definition_json in glob.glob(allele_definitions_path + "/*.json"):
    ...
    def  create_query_region(self):
        for gene, detail in self.allele_definition_set.items() :
            query = self.query_region(
                allele_definition=detail,
                ana_user_id=self.user_id,
                ana_id=self.id,
                vcf_gz_file=self.vcf_path)
            self.query_set[gene] = query
        # pass


    def check_null_dp(self,dp): 
        '''
        chech dp of the genome ,return dp else 0

        @param dp: varible given read by cyvcf
        '''
        if dp is not None:
            return dp
        else:
    ...
    def extract_genotype(self, gene, call_genotype):
        '''
    ...
    def extract_genotype_in_rage(self, gene, genotype_in_range):
        '''
        ...
        for genotype in genotype_in_range:
            a1, a2 = self.extract_genotype(gene, genotype)
            allele1 += a1
            allele2 += a2
        return f"{allele1}|{allele2}"

    def convert_ins(self,ref, allele):
    ...
    def convert_del(self,ref, allele):
        if ref == allele:
            """
            ref     allele      allele_convert
            ATCG    ATCG        TCG
    ...
    def convert_allele(self,hgvs_type, variant_type, is_del, ref, allele):
        '''
        ...
        if hgvs_type == "SNP":
            ...
            ## useless? rare case
                ...
                return self.convert_ins(ref, allele)
            elif variant_type == "indel" and is_del == True: #### CNV
                return self.convert_del(ref, allele)
            else:
    ...
    def sum_up_gt_phases(self,gt_phases, allele1, allele2):
        '''
    ...
    def sum_up_gene_phases(self,variants):
        '''
    ...
    def phase_checker(self,vcf_pro,matcer) :
    ...
    def query_region(self,allele_definition, ana_user_id, ana_id, vcf_gz_file):
        '''
        ...
        for variant in haplotype_template["variants"]:
            ...
            if variant["hgvs_type"] == "SNP" or variant["hgvs_type"] == "INS" or variant["hgvs_type"] == "CNV":
                ...
                for genome in vcf(region):
                    ...
                    v_vcf["dp"] = self.check_null_dp(genome.format("DP").tolist()[0][0]) if "DP" in genome.FORMAT else 0
                    ...
                    v_vcf["allele1"], v_vcf["allele2"] = self.extract_genotype(allele_definition["gene"], v_vcf["gt_bases"])
                    v_vcf["allele1_convert"] = "." if re.match(r"^(\.+)$", v_vcf["allele1"]) or re.match(r"^(\.+)$", v_vcf["allele2"]) else self.convert_allele(v_vcf["hgvs_type"], genome.var_type, genome.is_deletion, genome.REF, v_vcf["allele1"])
                    v_vcf["allele2_convert"] = "." if re.match(r"^(\.+)$", v_vcf["allele1"]) or re.match(r"^(\.+)$", v_vcf["allele2"]) else self.convert_allele(v_vcf["hgvs_type"], genome.var_type, genome.is_deletion, genome.REF, v_vcf["allele2"])
                    v_vcf["gt_phases"] = self.sum_up_gt_phases(genome.gt_phases[0], v_vcf["allele1"], v_vcf["allele2"])
                ...
                for genome in vcf(region):
                    ...
                    if genome.var_type == "indel" and genome.is_deletion == True:
                        ...
                        v_vcf["allele1"], v_vcf["allele2"] = self.extract_genotype(allele_definition["gene"], v_vcf["gt_bases"])
                        v_vcf["allele1_convert"] = "." if re.match(r"^(\.+)$", v_vcf["allele1"]) or re.match(r"^(\.+)$", v_vcf["allele2"]) else self.convert_allele(genome.REF, v_vcf["allele1"])
                        v_vcf["allele2_convert"] = "." if re.match(r"^(\.+)$", v_vcf["allele1"]) or re.match(r"^(\.+)$", v_vcf["allele2"]) else self.convert_allele(genome.REF, v_vcf["allele2"])
                        v_vcf["gt_phases"] = self.sum_up_gt_phases(genome.gt_phases[0], v_vcf["allele1"], v_vcf["allele2"])
                        ...
                        for inx, in_range in enumerate(range(int(variant['start']), int(variant['end']) + 1)):
                            ...
                            for genome in vcf(region):
                                ...
                                if in_range != int(genome.start) + 1:
                                    continue
                                if genome.var_type == "snp" or genome.var_type == "unknown":
                                    dp_in_range[inx] = self.check_null_dp(genome.format("DP").tolist()[0][0]) if "DP" in genome.FORMAT else 0
                        ...
                        v_vcf["gt_bases"] = self.extract_genotype_in_rage(allele_definition["gene"], genotype_in_range)
                        v_vcf["allele1"], v_vcf["allele2"] = self.extract_genotype(allele_definition["gene"], v_vcf["gt_bases"])
        ...
        allele_matcher["gene_phases"] = self.sum_up_gene_phases(allele_matcher["variants"])
...
vcf = glob.glob(smallchunk)[0]
matcher = AlleleMatcher(
    allele_definitions_path=defaults_allele_definitions_transform,
    vcf_path=vcf)
matcher.create_query_region()
#%%
